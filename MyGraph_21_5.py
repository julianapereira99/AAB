...
class MyGraph:
	...
	def search_hamiltonian_path_from_node(self, start):
		...
		while len(path) < len(self.get_nodes()):
			...
			if len(self.graph[current]) > nxt_index:  # vai avançando na árvore/grafo
				...
				if nxtnode not in path:
					...
					visited[nxtnode] = 0
					current = nxtnode
			else:  # vai retrocedendo na árvore/grafo
				if len(path) > 1:
					rmvnode = path.pop()  # remover o nó do path
					del visited[rmvnode]  # deletar o nó do visited (é como se nunca passa-se nesse nó)
					current = path[-1]  # current passa a ser o último valor de path.
				else: return None  # quando se retrocede tanto que apenas sobre o start então não existe caminho retornando None
		return path
